...
conn=mysql.connector.connect(host='localhost',user='root',password='password',charset="utf8")
cur=conn.cursor()
...
def changemasterpass():
	mas_change=input('Enter the new password     :')
	cur.execute("update user set Password='{}'".format(mas_change))
	conn.commit()
	print("Password changed to ",mas_change," !!!")

def changemasteruser():
	pos=input('Enter the new Username     :  ')
